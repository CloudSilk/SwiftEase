...
const TabManager = ({ children }: any) => {
  ...
  const {tabs, setTabs, setSearchCaches, searchCaches} = useModel('global');
  ...
  const onEdit = (targetKey: any, action: any) => {
    if (action === 'add') {
      ...
      const selectTab = tabs.find(pane => pane.id === targetKey);
      if(selectTab) {
        const str = selectTab.path.split('?')[1];
        const query = queryString.parse(str || '');
        const pageName = query?.pageName ?? '';
        if(searchCaches[pageName as string]) {
          searchCaches[pageName as string] = null;
          delete searchCaches[pageName as string];
