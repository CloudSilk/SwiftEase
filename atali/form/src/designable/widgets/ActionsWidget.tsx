...
import { Form as FormSchame } from "@swiftease/atali-pkg";
...
export const ActionsWidget = observer((props:any) => {
  ...
  const [formSchame, setFormSchame] = useState<FormSchame>({} as FormSchame)
  const [selectedVersion, setSelectedVersion] = useState<string>("0")
  useEffect(() => {
    loadInitialSchema(designer, props?.formID, (data: FormSchame) => {
      setFormSchame(data)
      const list: any[] = [{
        label: '当前版本',
        value: "0"
  ...
  return (
    <Space style={{ marginRight: 10 }}>
      <a>{formSchame?.group + '-' + formSchame?.pageName + '-' + formSchame?.name}</a>
      <div><a>版本选择：</a><Select style={{ width: 150 }} defaultValue={"0"} value={selectedVersion}
        options={versions} onSelect={(value:string, option:any) => {
